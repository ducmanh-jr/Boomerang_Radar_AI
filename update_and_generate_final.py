...
def update_markdown():
    ...
    skip = False
    for line in lines:
        if line.startswith("**Tác giả:**") or line.startswith("**Đơn vị:**") or line.startswith("**Ngày phát hành:**") or line.startswith("**Trạng thái hệ thống:**"):
            continue
        new_lines.append(line)
        
    with open(md_path, 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    print("Updated BaoCao_AI_DuDoanKhachHangQuayLai.md - Removed author/unit metadata.")
...
def generate_final_docx():
    ...
    # ==================== TRANG BÌA ĐƠN GIẢN ĐẸP (SẠCH SẼ - ĐÃ BỎ METADATA) ====================
    ...
    # ==================== PARSING MARKDOWN CONTENT (PURE TIMES NEW ROMAN BLACK ON WHITE) ====================
    with open(md_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    ...
    while i < len(lines):
        ...
        # Headings (Times New Roman Pure Black)
        if line.startswith('# '):
    ...
    doc.save(docx_path)
    print(f"Successfully generated final clean DOCX at: {docx_path}")
