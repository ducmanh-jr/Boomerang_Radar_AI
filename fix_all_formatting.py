...
def clean_markdown_math():
    md_path = r"c:\Users\Admin\ducmanh\DMC_kha-nang-khach-hang-quay-lai\BaoCao_AI_DuDoanKhachHangQuayLai.md"
    with open(md_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Clean pipe characters inside math formulas so they don't conflict with markdown table syntax
    content = content.replace("P(yᵢ = 1 | xᵢ)", "P(yᵢ = 1 ; xᵢ)")
    content = content.replace("(|Sᵥ| / |S|)", "(card(Sᵥ) / card(S))")
    content = content.replace("(|S|! · (|N| - |S| - 1)!) / |N|!", "(k! · (N - k - 1)!) / N!  [với k = card(S)]")
    content = content.replace("h(t | x)", "h(t ; x)")
    content = content.replace("S(t | x)", "S(t ; x)")
    content = content.replace("||w||", "||w||")
    
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Cleaned markdown math formulas - removed pipe ambiguities.")
...
def parse_inline_formatting(p, text, font_name="Times New Roman", font_size=11, font_color=RGBColor(0,0,0), is_bold=False):
    """
    Parses **bold**, *italic*, and `code` tags cleanly into Word Runs without printing raw asterisks or backticks!
    """
    pattern = r'(\*\*.*?\*\*|\*.*?\*|`.*?`)'
...
def generate_perfect_docx():
    ...
    parse_inline_formatting(p_title, "BÁO CÁO TỔNG QUAN HỆ THỐNG AI DỰ ĐOÁN KHẢ NĂNG KHÁCH HÀNG QUAY LẠI MUA HÀNG", font_size=22, is_bold=True)

    p_sub = doc.add_paragraph()
    p_sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_sub.paragraph_format.space_before = Pt(0)
    p_sub.paragraph_format.space_after = Pt(36)
    parse_inline_formatting(p_sub, "*CUSTOMER REPURCHASE PREDICTION AI SYSTEM*\nEnd-to-End Machine Learning Pipeline & Enterprise Architecture", font_size=13)
    ...
    parse_inline_formatting(p_toc_h, "MỤC LỤC TỔNG QUAN", font_size=16, is_bold=True)
    ...
    for idx, (title, page_str) in enumerate(toc_items):
        ...
        pt = c_t.paragraphs[0]
        parse_inline_formatting(pt, title, font_size=10.5 if is_main else 10, is_bold=is_main)
        
        pp = c_p.paragraphs[0]
        pp.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        parse_inline_formatting(pp, page_str, font_size=10.5 if is_main else 10, is_bold=is_main)
    ...
    parse_inline_formatting(p_fig_h, "DANH MỤC HÌNH VẼ (24 HÌNH MINH HỌA)", font_size=14, is_bold=True)
    ...
    for idx, (f_num, f_desc) in enumerate(figures_list):
        ...
        pn = c_n.paragraphs[0]
        parse_inline_formatting(pn, f_num, font_size=9.5, is_bold=True)
        
        pd = c_d.paragraphs[0]
        parse_inline_formatting(pd, f_desc, font_size=9.5)
    ...
    while i < len(lines):
        ...
        # Smart Markdown Table detection: Line MUST start with '|' AND contain '|'
        if line.strip().startswith('|') and line.strip().endswith('|') and '|' in line[1:-1]:
            table_lines.append(line)
            in_table = True
            i += 1
            continue
        else:
            if in_table and len(table_lines) > 0:
                # Process collected table
                table_data = []
                for t_line in table_lines:
                    parts = [c.strip() for c in t_line.split('|')[1:-1]]
                    ...
                    for r_idx, row_vals in enumerate(table_data):
                        for c_idx, val in enumerate(row_vals):
                            if c_idx < cols:
                                ...
                                parse_inline_formatting(p, val, font_size=10, is_bold=(r_idx == 0))
        ...
        if img_match:
            ...
            if os.path.exists(img_abs_path):
                ...
                if i + 1 < len(lines) and lines[i+1].strip().startswith('*Hình'):
                    ...
                    parse_inline_formatting(p_cap, caption_line.strip('*'), font_size=9.5)
                    i += 1
            i += 1
            continue
        ...
        if line.startswith('# '):
            ...
            p.paragraph_format.space_after = Pt(8)
            parse_inline_formatting(p, line[2:], font_size=18, is_bold=True)
        elif line.startswith('## '):
            p = doc.add_paragraph(style='Heading 2')
            p.paragraph_format.space_before = Pt(14)
            p.paragraph_format.space_after = Pt(6)
            parse_inline_formatting(p, line[3:], font_size=14, is_bold=True)
        elif line.startswith('### '):
            p = doc.add_paragraph(style='Heading 3')
            p.paragraph_format.space_before = Pt(12)
            p.paragraph_format.space_after = Pt(4)
            parse_inline_formatting(p, line[4:], font_size=12, is_bold=True)
        elif line.startswith('#### '):
            p = doc.add_paragraph()
            p.paragraph_format.space_before = Pt(10)
            p.paragraph_format.space_after = Pt(4)
            parse_inline_formatting(p, line[5:], font_size=11, is_bold=True)
        elif line.startswith('* ') or line.startswith('- '):
            p = doc.add_paragraph(style='List Bullet')
            p.paragraph_format.space_before = Pt(0)
            p.paragraph_format.space_after = Pt(4)
            p.paragraph_format.line_spacing = 1.15
            parse_inline_formatting(p, line[2:], font_size=11)
            ...
            p.paragraph_format.space_after = Pt(4)
            p.paragraph_format.line_spacing = 1.15
            parse_inline_formatting(p, content, font_size=11)
        else:
            p = doc.add_paragraph()
            p.paragraph_format.space_before = Pt(0)
            p.paragraph_format.space_after = Pt(6)
            p.paragraph_format.line_spacing = 1.15
            parse_inline_formatting(p, line, font_size=11)

        i += 1

    doc.save(docx_path)
    print(f"Successfully generated perfect Word report at: {docx_path}")

if __name__ == '__main__':
    clean_markdown_math()
    generate_perfect_docx()
